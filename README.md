...
- **设置**：在同一页面管理手机传感器、电脑音视频和外部设备。手机实验可从实验库直接进入设置，完成连接与绑定后再进入测量。外部设备按各自协议填写参数，再关联实验输入或输出。
- **数据与回放**：保存快照、确认后恢复，使用表单映射导入 CSV/TSV/WAV，或查看不驱动设备的独立回放。
- **电脑音视频设置**：手动授权浏览器麦克风、相机与扬声器；Windows 原生媒体放在同页高级选项。进入设置不会自动打开或播放。

普通数据导入表单的列号从 **1** 开始，高级 JSON 的索引仍从 **0** 开始。保存、恢复、导出及原版界面的对齐边界见 [新版界面使用说明](docs/USER-INTERFACE.md)；首次使用也可打开页面右上方的帮助。

## 手机浏览器接入（验证版）

「设置」中的手机传感器区域提供完全离线的局域网扫码入口。默认直接打开本机 HTTPS 页面并按浏览器提示授权，手动安装证书仅为可选兼容方式。当前支持单个手机输入的运动数据、PCM 和普通图像；用户反馈 Android Chrome 与 iPhone Safari 均可申请权限，真实设备的完整实验验收尚未完成。

从实验库选择“手机麦克风、手机相机颜色、手机运动”即可开始。操作见 [手机实验快速开始](docs/PHONE-QUICKSTART.md)；遇到浏览器访问或授权问题时，参考 [首次连接指南](docs/PHONE-FIRST-CONNECTION.md) 和 [浏览器来源验证记录](docs/validation/phone-browser-origin.md)。
